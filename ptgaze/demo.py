...
class Demo:
    ...
    def _process_image(self, image) -> None:
        ...
        for face in faces:
            ...
            now_pose_list = self._draw_head_pose(face)
            # 调用 _draw_gaze_vector 并获取当前帧的注视方向列表
            gaze_directions = self._draw_gaze_vector(face)
    ...
    def _save_frame_data(self, video_name: str) -> None:
        output_path = pathlib.Path(self.config.demo.output_dir) / f'{video_name}.json'
    ...
    def _draw_head_pose(self, face: Face) -> list:
        ...
        now_pose_list=[pitch,yaw,roll] # 将当前帧pitch, yaw, roll存成列表
        # logger.info(f'[head] pitch: {pitch:.2f}, yaw: {yaw:.2f}, '
        #             f'roll: {roll:.2f}, distance: {face.distance:.2f}')
        logger.info(f'pose_list_{now_pose_list}')
        return now_pose_list
    ...
    def _draw_gaze_vector(self, face: Face) -> list:
        ...
        if self.config.mode == 'MPIIGaze':
            for key in [FacePartsName.REYE, FacePartsName.LEYE]:
                ...
                gaze_directions.append((pitch, yaw))
                logger.info(
                    f'gaze_directions[{key.name.lower()}] ')
